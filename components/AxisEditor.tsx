...
export function AxisEditor({
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      {tableType === "date" ? (
        <div>
          ...
          <div className="space-y-2 sm:space-y-3">
            <div className="grid grid-cols-2 sm:grid-cols-[minmax(0,1fr)_9rem] gap-2 sm:gap-3">
              ...
              <div className="grid grid-cols-5 gap-1.5 sm:gap-2">
                {([
                  ...
                  <button
                    ...
                    className={`min-w-0 px-1 sm:px-2 py-2 rounded-lg text-xs sm:text-sm font-medium border transition-colors whitespace-nowrap ${
      ...
      {tableType === "date" ? null : (
        <div>
          <div className="flex items-center justify-between gap-2 mb-2 sm:mb-3">
            ...
            <div className="grid grid-cols-2 rounded-lg border border-gray-200 overflow-hidden text-xs w-auto">
              {(["date", "weekday"] as const).map((mode, idx) => (
                <button
                  key={mode}
                  type="button"
                  onClick={() => switchColMode(mode)}
                  className={`px-3 sm:px-4 py-1.5 font-medium transition-colors ${idx > 0 ? "border-l border-gray-200" : ""} ${
      ...
      {/* ── Preview ── */}
      <div className="hidden sm:block rounded-xl border border-gray-200 p-3">
