...
const faqs = [
  ...
  {
    q: "イベントを削除するには？",
    a: "編集URLまたはパスワードで認証後、「イベントを編集する」→「イベント設定の編集」ページ下部の「イベントを削除する」ボタンから削除できます。削除すると回答・コメントを含むすべてのデータが失われ、取り消しできません。",
  },
  {
    q: "パスワードを忘れた場合は？",
    a: "残念ながら、設定済みのパスワードを復元する方法はありません。編集URLを保存している場合はそちらでアクセスしてください。編集URLもない場合は、イベントの編集・削除が行えなくなります。次回からは必ず編集URLを安全な場所に保管してください。",
  },
  {
    q: "参加者の上限を変更できますか？",
    a: "はい。編集URLまたはパスワードで認証後、「イベント設定の編集」ページの「参加者上限」欄から変更できます。現在の参加者数未満に下げることはできません（上げることは1〜50名の範囲内で可能）。",
  },
  {
    q: "カレンダーに予定を追加するには？",
    a: "集計画面でセルをタップすると詳細モーダルが開き、Google・Yahoo・Outlook・Appleカレンダーへの追加ボタンが表示されます。ボタンをタップするとお使いのカレンダーアプリに日程が自動入力された状態で開きます。",
  },
  {
    q: "モバイルで使えますか？",
    a: "はい、スマートフォン・タブレットに対応しています。iOS Safari・Android Chromeで動作確認済みです。ホーム画面に追加（PWAインストール）することで、アプリのように使うことも可能です。",
  },
  {
    q: "音声入力はどうやって使うの？",
    a: "回答画面の「音声入力」ボタンをタップしてマイクへのアクセスを許可し、参加できる日程を声で話すと、AIが自動で該当セルを選択します。例：「5月1日と2日の午前は参加できます」のように話しかけてください。",
  },
  {
    q: "画像から予定を読み込む機能の使い方は？",
    a: "回答画面の「画像から読込」ボタンをタップし、カレンダーアプリのスクリーンショットや手書きのスケジュール画像を選択します。AIが画像内の日程・時間帯を読み取り、自動でセルを選択します。読み取り精度は画像の鮮明さや記載形式によって異なります。",
  },
];
